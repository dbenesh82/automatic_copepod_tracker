...
The slope of the correlation gets closer to 1, the intercept closer to zero, and R^2^ increases to 0.76, all of which can be considered an improvement. This is not to say that all mistakes are eliminated, but enough are to make tedious manual tracking and fast automatic tracking comparable.
...
ggplot(bd_avg, aes(x = sec_pooled, y = dist_a, color = trt)) + geom_path() + 
  labs(color = "Treatment", title = "Auto tracking")
...
ggplot(bd_avg, aes(x = slice, y = dist_a, color = trt)) + geom_path() + 
  labs(color = "Treatment", title = "Manual tracking") +
  scale_x_continuous(breaks = seq(0, 120, 30))
```
